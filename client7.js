...
function init() {
  ...
  for (let i = 0, l = position.count; i < l; i++) {
    color.setHSL(Math.random() * 0 + 10, 10, Math.random() * 0.25 + 0.75);
  ...
  plane.position.set(-50 , 15 , -15);
  // Place plane geometry
  scene.add( plane );

  // Second Image (Text with image and white background)
  // Load image as texture
  const texture2 = new THREE.TextureLoader().load( './assets/m.png' );
  // immediately use the texture for material creation
  ...
  // Position plane geometry
  plane2.position.set(30 , 90 , -200);
  // Place plane geometry
  scene.add( plane2 );

  // Third Image (Text with image and white background)
  // Load image as texture
  const texture3 = new THREE.TextureLoader().load( './assets/mf.png' );
  // immediately use the texture for material creation
  const material3 = new THREE.MeshBasicMaterial( { map: texture3, side: THREE.DoubleSide } );
  // Create plane geometry
  const geometry3 = new THREE.PlaneGeometry( 200, 160 );
  // Apply image texture to plane geometry
  const plane3 = new THREE.Mesh( geometry3, material3);
  // Position plane geometry
  plane3.position.set(-180 , 100 , 100);
  // Place plane geometry
  scene.add( plane3 );


  // Fourth Image (Text with image and white background)
  // Load image as texture
  const texture4 = new THREE.TextureLoader().load( './assets/d.png' );
  // immediately use the texture for material creation
  const material4 = new THREE.MeshBasicMaterial( { map: texture4, side: THREE.DoubleSide } );
  // Create plane geometry
  const geometry4 = new THREE.PlaneGeometry( 200, 160 );
  // Apply image texture to plane geometry
  const plane4 = new THREE.Mesh( geometry4, material4);
  // Position plane geometry
  plane4.position.set(280 , 100 , -20);
  // Place plane geometry
  scene.add( plane4 );

  const texture5 = new THREE.TextureLoader().load( './assets/brain.jpg' );
  // immediately use the texture for material creation
  const material5 = new THREE.MeshBasicMaterial( { map: texture5, side: THREE.DoubleSide } );
  // Create plane geometry
  const geometry5 = new THREE.PlaneGeometry( 200, 100 );
  // Apply image texture to plane geometry
  const plane5 = new THREE.Mesh( geometry5, material5 );
  // Position plane geometry
  plane5.position.set(-50 , 60 , -100);
  // Place plane geometry
  scene.add( plane5 );
...
/// YOUTUBE video
  ...
  // 				iframe.src = [ 'https://www.youtube.com/watch?v=KaR2L3z6gJs', id, '?rel=0' ].join( '' );
...
/////model
  ...
  const loader = new GLTFLoader().load(
    "./assets/brain.glb", // comment this line out and un comment the line below to swithc models
